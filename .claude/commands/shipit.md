...
- DNS lives in Cloudflare. `niv-coffee.danielhadar.com` is a CNAME record pointing to `danielhadar.github.io`. Pages reads the domain from the `CNAME` file in the repo root.
- The two **QR poster** pages (`qr-code.html` for the punch card, `deal/deal-qr.html` for the deal app) encode hardcoded full URLs (`https://niv-coffee.danielhadar.com/` and `.../deal/`). If the domain ever changes, both must be updated alongside `CNAME`.
