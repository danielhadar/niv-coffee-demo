...
- The two **QR poster** pages (`qr-code.html` for the punch card, `deal-qr.html` for the deal app) encode hardcoded full URLs (`https://niv-coffee.danielhadar.com/` and `.../deal/`). If the domain ever changes, both must be updated alongside `CNAME`.
